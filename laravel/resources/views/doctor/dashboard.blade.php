...
<div class="grid grid-cols-1 md:grid-cols-3 gap-8">
    ...
    <div class="md:col-span-2">
        ...
        <div class="glass-card p-6 mb-8">
            <h1 class="text-3xl font-bold text-gray-800">Welcome back, Dr. {{ $doctor->user->name ?? 'N/A' }}!</h1>
            <p class="mt-2 text-lg text-gray-600">Here's a summary of your activities today.</p>
        </div>

        <!-- Quick Stats -->
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
            <div class="glass-card p-5 glow-on-hover">
                <div class="flex items-center">
                    <div class="bg-blue-500/20 text-blue-600 p-4 rounded-full">
                        <i class="fas fa-calendar-check fa-2x"></i>
                    </div>
                    <div class="ml-4">
                        <h3 class="text-2xl font-bold text-gray-800">{{ $doctor->appointments->where('cStatus', 'Scheduled')->count() }}</h3>
            ...
            <div class="glass-card p-5 glow-on-hover">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <h3 class="text-2xl font-bold text-gray-800">{{ $doctor->appointments->where('cStatus', 'Completed')->count() }}</h3>
            ...
            <div class="glass-card p-5 glow-on-hover">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <h3 class="text-2xl font-bold text-gray-800">{{ $doctor->appointments->where('cStatus', 'Cancelled')->count() }}</h3>
                         ...
                         @forelse($patients as $patient)
                            <tr>
                                <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-800 sm:pl-0">
                                    <a href="{{ route('doctor.patients.history', ['patient' => $patient]) }}" class="hover:text-blue-600">
                                        {{ $patient->cName }}
                                    </a>
                                </td>
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-center text-gray-600">{{ $patient->appointments_count }}</td>
                                <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-0">
                                    <a href="{{ route('doctor.patients.history', ['patient' => $patient]) }}" class="text-blue-600 hover:text-blue-900">View History</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
    ...
    <div class="md:col-span-1">
        <div class="glass-card p-6">
            ...
            <ul class="space-y-4">
                @forelse($doctor->appointments->where('cStatus', 'Scheduled')->sortBy('dAppointmentDateTime')->take(5) as $appointment)
                <li class="border border-gray-200/30 rounded-lg p-4 hover:shadow-md transition-shadow">
                    <div class="flex justify-between items-center">
                        <div>
                            <p class="font-semibold text-gray-800">{{ $appointment->patient->cName ?? 'N/A' }}</p>
                            <p class="text-sm text-gray-600">{{ \Carbon\Carbon::parse($appointment->dAppointmentDateTime)->format('g:i A') }}</p>
                        </div>
                        <p class="text-sm text-gray-500">{{ \Carbon\Carbon::parse($appointment->dAppointmentDateTime)->format('M d') }}</p>
                    </div>
                </li>
                @empty
